...
BEGIN_MESSAGE_MAP(Cdemo_MFCDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &Cdemo_MFCDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void Cdemo_MFCDlg::showBvhBoneInfo(SOCKET_REF sender, BvhDataHeader* header, float* data)
{
	...
	for (int curSel = 38; curSel < BVHBoneCount; curSel++) {
		if (header->WithDisp)
		{
			...
			if (curSel != BVHBoneCount - 1)row -= 1;
			success = csv.SetCell(row, 1 + (curSel - 38) * 3, std::to_string(angX).c_str());
			success = csv.SetCell(row, 2 + (curSel - 38) * 3, std::to_string(angY).c_str());
			success = csv.SetCell(row, 3 + (curSel - 38) * 3, std::to_string(angZ).c_str());
...
void Cdemo_MFCDlg::OnBnClickedButton1()
{
	// TODO: Add your control notification handler code here
	CString BoneID;
	CkString strOut;
	CkCsv csv;
	bool success = csv.SetColumnName(0, "label");
	//CString BoneID;
	for (int i = 38; i < BVHBoneCount; i++)
	{

		const string bone = "Bone" + std::to_string(i), x=bone+"_x", y = bone + "_y", z = bone + "_z";
		
			success = csv.SetColumnName(1+(i-38)*3 , x.c_str());
			success = csv.SetColumnName(2 + (i - 38) * 3, y.c_str());
			success = csv.SetColumnName(3 + (i - 38) * 3, z.c_str());
